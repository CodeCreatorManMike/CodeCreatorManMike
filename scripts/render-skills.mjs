...
export function buildSkillsGroup(yOffset = 0) {
  ...
  const header = `
  <text x="24" y="${Y(34)}" font-size="20" font-weight="bold" fill="${PALETTE.accent}" font-family="${FONT_FAMILY}">&gt; skills --progress</text>
  <line x1="24" y1="${Y(46)}" x2="${width - 24}" y2="${Y(46)}" stroke="${PALETTE.muted}" stroke-width="1" opacity="0.4" />`;

  const footer = `<text x="24" y="${Y(height - 16)}" font-size="11" fill="${PALETTE.muted}" font-family="${FONT_FAMILY}">learning progress • hand-tracked, updated manually in scripts/config.mjs</text>`;
